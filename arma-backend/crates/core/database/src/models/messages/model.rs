...
impl Message {
    ...
    pub async fn create_from_api(
        ...
        if allow_mass_mentions && !role_mentions.is_empty() {
            if let Some(ref server_id_str) = server_id {
                let server_data = db
                    .fetch_server(server_id_str.as_str())
                    .await
            ...
            && (mentions_everyone || mentions_online || !role_mentions.is_empty())
        {
            ...
            if let Some(user) = match author {
                ...
                let owned_user: User = match user {
                    Ok(user) => user.to_owned().into(),
                    Err(_) => return Err(create_error!(InvalidProperty)),
                };
