...
pub static RE_USERNAME: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"^(\p{L}|[\d_.-])+$")
        .expect("Failed to compile username regex - invalid pattern")
});
...
pub static RE_DISPLAY_NAME: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"^[^\u200B\n\r]+$")
        .expect("Failed to compile display name regex - invalid pattern")
});
