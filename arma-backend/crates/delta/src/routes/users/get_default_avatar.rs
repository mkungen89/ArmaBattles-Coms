...
pub async fn default_avatar(target: String) -> CachedFile {
    CachedFile((
        ...
        match target.chars().last() {
            Some('0') | Some('1') | Some('2') | Some('3') | Some('S') | Some('Z') => {
                include_bytes!("avatars/2.png").to_vec()
            }
            Some('4') | Some('5') | Some('6') | Some('7') | Some('T') => {
                include_bytes!("avatars/3.png").to_vec()
            }
            Some('8') | Some('9') | Some('A') | Some('B') => {
                include_bytes!("avatars/4.png").to_vec()
            }
            Some('C') | Some('D') | Some('E') | Some('F') | Some('V') => {
                include_bytes!("avatars/5.png").to_vec()
            }
            Some('G') | Some('H') | Some('J') | Some('K') | Some('W') => {
                include_bytes!("avatars/6.png").to_vec()
            }
            Some('M') | Some('N') | Some('P') | Some('Q') | Some('X') => {
                include_bytes!("avatars/7.png").to_vec()
            }
            // Default avatar for None (empty string) or any other character
