...
pub async fn message_send(
    ...
    let allow_mentions = if let Some(server) = query.server_ref() {
        if server.discoverable {
            // Parse user ID as ULID to get account age
            match ulid::Ulid::from_string(&user.id) {
                Ok(ulid) => {
                    (Utc::now() - ulid.datetime()) >= Duration::hours(12)
                }
                Err(_) => {
                    // If we can't parse the user ID, default to allowing mentions
                    // This should never happen in practice as user IDs are always valid ULIDs
                    true
                }
            }
        } else {
