...
pub async fn web() -> Rocket<Build> {
    ...
    let db = revolt_database::DatabaseInfo::Auto
        .connect()
        .await
        .expect("Failed to connect to database - ensure MongoDB is running and accessible");
    log::info!("database_here {db:?}");
    db.migrate_database()
        .await
        .expect("Failed to run database migrations - database may be corrupted");
    ...
    let cors = CorsOptions {
        ...
        .iter()
        .map(|s| FromStr::from_str(s).expect("Failed to parse HTTP method - invalid method string"))
        .collect(),
        expose_headers: [
            "X-Ratelimit-Limit",
            "X-Ratelimit-Bucket",
