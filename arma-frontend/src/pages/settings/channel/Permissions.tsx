    ...
    return (
        <PermissionsLayout
            ...
            editor={({ selected }) => {
                ...
                function save() {
                    // Prevent users from denying themselves ViewChannel permission
                    if (typeof currentValue === "object") {
                        const ViewChannelBit = 1; // ViewChannel permission bit
                        const isDenied = (currentValue.d & ViewChannelBit) === ViewChannelBit;

                        if (isDenied && channel.server?.member?.roles?.includes(selected)) {
                            alert("You cannot deny yourself the ViewChannel permission. This would hide the channel from you.");
                            return;
                        }
                    }
