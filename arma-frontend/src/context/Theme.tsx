...
export const PRESETS: Record<string, Theme> = {
    light: {
        ...
        "primary-header": "#E8F2FC",
        "secondary-background": "#F1F1F1",
        "secondary-foreground": "#1A1A1A",
        "secondary-header": "#F1F1F1",
        "tertiary-background": "#2C5F2D",
        "tertiary-foreground": "#2D2D2D",
        "status-online": "#2C5F2D",
        "status-away": "#F39F00",
        "status-focus": "#4A90E2",
        "status-busy": "#E24A4A",
        "status-streaming": "#4A90E2",
...
export default observer(() => {
    ...
    useEffect(() => {
        root.setProperty("--monospace-font", `"${currentMonoFont}"`);
        try {
            MONOSPACE_FONTS[currentMonoFont]?.load();
        } catch (err) {
            console.error(`Failed to load monospace font: ${currentMonoFont}`);
        }
    }, [currentMonoFont]);
